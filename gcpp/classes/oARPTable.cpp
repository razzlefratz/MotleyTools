...
void ARP::ipin (const void * memory, unsigned extent)

{
	struct ethernet_header * ethernet_header = (struct ethernet_header *) (memory);
	struct internet_packet * internet_packet = (struct internet_packet *) (memory);
...
unsigned ARP::in (const void * memory, unsigned extent)

{
	struct ethernet_header * ethernet_header = (struct ethernet_header *) (memory);
	struct arp_packet * arp_packet = (struct arp_packet *) (memory);
	struct arp_header * arp_header = (struct arp_header *) (& arp_packet->arp_header);
	if (extent < sizeof (* arp_packet))
	{
	}
	else if (ntohs (arp_header->opcode) == ARP_REQUEST)
	{
		if (! std::memcmp (arp_header->target_ipaddr, internet.address.binary (), sizeof (arp_header->target_ipaddr)))
		{
			...
			std::memcpy (ethernet_header->target_address, arp_header->source_hwaddr, sizeof (ethernet_header->target_address));
			std::memcpy (ethernet_header->source_address, ethernet.address.binary (), sizeof (ethernet_header->source_address));
			std::memcpy (arp_header->target_hwaddr, arp_header->source_hwaddr, sizeof (arp_header->target_hwaddr));
...
unsigned ARP::out (const void * memory, unsigned extent)
...
static void PrintARPHeader (const void * header, unsigned extent)
...
/*====================================================================*
 *
 *   boot empty (const void * memory, unsigned extent);
 *
...
bool ARP::empty (const void * memory, unsigned extent)
