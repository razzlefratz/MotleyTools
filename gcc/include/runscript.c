...
{
	char script [FILENAME_MAX + 1] = _PATH_TEMPFILE;
	char header [FILENAME_MAX + 4] = "#!" _PATH_BSHELL "\n";
	pid_t pid;
	int fd;
	...
	if (pid == (pid_t) (0)) 
	{
		char *argv [ARGVSIZE];
		int argc = 0;
		argv [argc++] = _PATH_BSHELL;
		argv [argc++] = script;
		argv [argc++] = (char *) (option);
		argv [argc++] = (char *) (0);
		execv (*argv, argv);
		error (1, errno, "%s: Can't start shell %s", script, *argv);
	}
	if (pid < (pid_t) (0)) 
	{
		error (0, errno, "%s: Can't spawn child", script);
		unlink (script);
		return (-1);
	}
	if (pid > (pid_t) (0)) 
