...
static int likeatty (char const *filename) 
...
/*====================================================================*
 *
 *   void broadcast_sigalrm (signo_t number);
 *
...

{
	...
	*old;
	struct stat statinfo;
	struct utmp *utmp;
	char banner [BROADCAST_L_BANNER] = "";
	size_t length = 0;
	size_t count = 0;
	...
	if ((flags & BROADCAST_B_BANNER) || getuid ()) 
	{
		...
		length += snprintf (banner + length, sizeof (banner) - length, "Broadcast from %s@%s ", username, hostname);
		length += strftime (banner + length, sizeof (banner) - length, "on %a %d %b %Y at %T %Z", localtime (&now));
 ...
 */
		...
		if (strcmp (new->name, ttyname (STDIN_FILENO)) == 0) 
		{
 ...
 */
		...
		if (!fork ()) 
		{
			...
			sigalrm.sa_handler = broadcast_sigalrm;
			sigalrm.sa_flags = SA_ONESHOT;
