...
#ifndef HTMLSCAN_SOURCE
#define HTMLSCAN_SOURCE
 ...
 *
 *   span nmtoken as per w3c html 1.0 specification;
 *   
 *.  Motley Tools by Charles Maier
 *:  Published 1982-2005 by Charles Maier for personal use
 *;  Licensed under the Internet Software Consortium License
...
/*====================================================================*
 *
 *   char * content (char * string, unsigned * lineno, char final);
 *
 *   span and normalize content as per w3c html 1.0 specification;
 *
 *.  Motley Tools by Charles Maier
 *:  Published 1982-2005 by Charles Maier for personal use
 *;  Licensed under the Internet Software Consortium License
...
static char * content (char *string, unsigned * lineno, char final) 
 ...
 *
 *   span comment as per w3c html 1.0 specification;
 *
 *.  Motley Tools by Charles Maier
 *:  Published 1982-2005 by Charles Maier for personal use
 *;  Licensed under the Internet Software Consortium License
...
#define GROUP_ELEMENTS 4
#define EMPTY_ELEMENTS 26

static char const * GROUP_ELEMENT [GROUP_ELEMENTS] = 
...
static char const * EMPTY_ELEMENT [EMPTY_ELEMENTS] = 
	...
	while (*string) 
	{
		if (*string == '<') 
		{
			...
			if ((prefix == '%') || (prefix == '?')) 
			{
				while ((*string) && (*string != '>')) 
				{
					string = context (string, &lineno, prefix);
					string++;
				}
			}
 ...
 */
			...
			{
				if (svindex (element->text, GROUP_ELEMENT, GROUP_ELEMENTS, strcmp) == GROUP_ELEMENTS) 
				{
					if (!strcmp (element->text, section->text)) 
					{
						error (1, 0, "Element <%s> on line %d terminated by <%s> on line %d", section->text, section->line, element->text, element->line);
					}
				}
				if (svindex (element->text, EMPTY_ELEMENT, EMPTY_ELEMENTS, strcmp) != EMPTY_ELEMENTS) 
				{
					continue;
				}
				section = element;
