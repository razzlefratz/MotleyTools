/*====================================================================*
 *
 *   void _qsort (void *list[], size_t listsize, int comp(void const *, void const *), void swap (void const *, void const *));
 *
 *   _stdlib.h
 *
...

{
	...
	if (pivot > 0) 
	{
		swap (&list [0], &list [pivot]);
		pivot = lower;
		while (++pivot < upper) 
		{
			if (comp (list [pivot], list [0]) < 0) 
			{
				swap (&list [pivot], &list [++lower]);
			}
		}
		pivot = upper;
		while (--pivot > lower) 
		{
			if (comp (list [pivot], list [0]) > 0) 
			{
				swap (&list [pivot], &list [--upper]);
			}
		}
		swap (&list [0], &list [pivot]);
		_qsort (list + upper, size - upper, comp, swap);
		_qsort (list, lower, comp, swap);
	}
	return;
}
