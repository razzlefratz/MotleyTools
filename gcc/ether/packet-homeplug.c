...

{
	...
	{
		2,
		"Normal retries based on specification"
	},
	{
		3,
		"Reserved"
...
{
	{
		HOMEPLUG_NS_ICID5130A1,
		HOMEPLUG_NS_ICID5130A1,
		"INT5130A1"
	},
	{
		HOMEPLUG_NS_ICID51X1USB,
		HOMEPLUG_NS_ICID51X1USB,
		"INT51X1 (USB Option)"
	},
	{
		HOMEPLUG_NS_ICID51X1PHY,
		HOMEPLUG_NS_ICID51X1PHY,
		"INT51X1 (PHY Option)"
	},
	{
		HOMEPLUG_NS_ICID51X1HOST,
		HOMEPLUG_NS_ICID51X1HOST,
		"INT51X1 (Host/DTE Option)"
	},
	{
		HOMEPLUG_NS_ICID5130A2,
		HOMEPLUG_NS_ICID5130A2,
		"INT5130A2"
	},
	{
		HOMEPLUG_NS_ICID_RSVD_MIN,
		HOMEPLUG_NS_ICID_RSVD_MAX,
		"Reserved"
	},
	{
		0,
...
{
	{
		HOMEPLUG_CER_MOD_ROBO,
		"ROBO Modulation"
	},
	{
		HOMEPLUG_CER_MOD_DBPSK,
		"DBPSK Modulation"
	},
	{
		HOMEPLUG_CER_MOD_DQPSK,
		"DQPSK Modulation"
	},
	{
		HOMEPLUG_CER_MOD_RSVD,
		"Reserved"
	},
	{
		0,
...
/* Extract Number Of MAC Data Entries */
	...
	homeplug_ne = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_MCTRL_NE;
...

{
	...
	homeplug_metype = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_MEHDR_METYPE;
...

{
	...
	ptvcursor_push_subtree (cursor, it, ett_homeplug_cer);
	{
		...
		BP = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_CER_BP;
		...
		if (BP) 
		{
			iNBDA = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_CER_NBDAS;
...

{
	...
	{
		mid = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor));
		length = tvb_get_ntohs (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor) + 1);
		switch (mid) 
		{
		...
		case HOMEPLUG_MID_LWM:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length == 7) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_data_address, 4, ENC_BIG_ENDIAN);
			dlen = tvb_get_ntohs (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor));
			ptvcursor_add (cursor, hf_homeplug_loader_data_length, 2, ENC_BIG_ENDIAN);
			if (length != 7) ptvcursor_add (cursor, hf_homeplug_loader_data, dlen, ENC_NA);
			break;
		case HOMEPLUG_MID_LRM:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length != 6) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_data_address, 4, ENC_BIG_ENDIAN);
			dlen = tvb_get_ntohs (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor));
			ptvcursor_add (cursor, hf_homeplug_loader_data_length, 2, ENC_BIG_ENDIAN);
			if (length != 6) ptvcursor_add (cursor, hf_homeplug_loader_data, dlen, ENC_NA);
			break;
		...
		case HOMEPLUG_MID_GMD:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length != 10) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
		...
		case HOMEPLUG_MID_SMD:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length == 13) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_module_id, 4, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_module_offset, 4, ENC_BIG_ENDIAN);
			dlen = tvb_get_ntohs (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor));
			if (length != 13) 
			...
			else ptvcursor_add (cursor, hf_homeplug_loader_module_size, 4, ENC_BIG_ENDIAN);
			break;
		case HOMEPLUG_MID_CM:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length == 5) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_module_id, 4, ENC_BIG_ENDIAN);
			break;
		case HOMEPLUG_MID_LEMD:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length == 9) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_module_id, 4, ENC_BIG_ENDIAN);
			break;
		...
		case HOMEPLUG_MID_SNVMP:
			...
			ptvcursor_add (cursor, hf_homeplug_loader_length, 2, ENC_BIG_ENDIAN);
			if (length == 17) ptvcursor_add (cursor, hf_homeplug_loader_status, 1, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_nvmp_type, 4, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_nvmp_pagesize, 4, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_nvmp_blocksize, 4, ENC_BIG_ENDIAN);
			ptvcursor_add (cursor, hf_homeplug_loader_nvmp_memorysize, 4, ENC_BIG_ENDIAN);
...

{
	...
	ptvcursor_push_subtree (cursor, it, ett_homeplug_hreq);
	{
		...
		case HOMEPLUG_MID_RESET:
			...
			if (length == 4) ptvcursor_add (cursor, hf_homeplug_hreq_reset_delay, 2, ENC_BIG_ENDIAN);
			break;
		default:
...
/* Append Basic/Extender specifier to info column */
	...
	if (check_col (pinfo->cinfo, COL_INFO)) col_append_str (pinfo->cinfo, COL_INFO, extended? " Extended": " Basic");
	if (!ptvcursor_tree (cursor)) return;
	ptvcursor_add_with_subtree (cursor, hf_homeplug_ns, homeplug_melen, FALSE, ett_homeplug_ns);
	{
		ti = proto_tree_add_boolean (ptvcursor_tree (cursor), hf_homeplug_ns_extended, ptvcursor_tvbuff (cursor), 0, 0, extended);
		...
		ns_bytes40 = tvb_get_letohs (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor));
		proto_tree_add_text (ptvcursor_tree (cursor), ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor), 2, "MHz :  %.3f", (float)(ns_bytes40)/42);
		...
		while (iTone < 15) 
		{
			newt_da = ((gint64)tvb_get_ntoh24 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor))) << 24;
			newt_da |= tvb_get_ntoh24 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)+3);
			if (newt_da != HOMEPLUG_ADDR_INEXISTANT) 
			{
				ptvcursor_add_text_with_subtree (cursor, SUBTREE_UNDEFINED_LENGTH, ett_homeplug_tone, "Tone Map #%d", iTone+1);
				{
			...
			{
				proto_tree_add_text (ptvcursor_tree (cursor), ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor), 12, "Tone Map #%d does not exist", iTone+1);
			}
			iTone++;
		}
		if (extended) 
		{
			while (i_buffer < 6) 
			{
				ptvcursor_add_text_with_subtree (cursor, SUBTREE_UNDEFINED_LENGTH, ett_homeplug_tx_bfr_state, "TX_BFR_%d_STATE", i_buffer);
				{
...

{
	...
	ptvcursor_push_subtree (cursor, it, ett_homeplug_bcn);
	{
		...
		fbn = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_BCN_FBN;
		ptvcursor_add (cursor, hf_homeplug_bcn_fbn, 1, ENC_BIG_ENDIAN);
		while (i_bridge < 2) 
		{
			ptvcursor_add_text_with_subtree (cursor, SUBTREE_UNDEFINED_LENGTH, ett_homeplug_bridge, "Bridge #%d", fbn + i_bridge);
			{
...

{
	gboolean network;
	if (!ptvcursor_tree (cursor)) return;
	network = tvb_get_guint8 (ptvcursor_tvbuff (cursor), ptvcursor_current_offset (cursor)) & HOMEPLUG_BC_NETWORK;

/* Append Network/Local specifier to info column */

	if (check_col (pinfo->cinfo, COL_INFO)) col_append_str (pinfo->cinfo, COL_INFO, network? " Network": " Local");

/* Call specific dissector */

	if (network) dissect_homeplug_bcn (cursor);
	else dissect_homeplug_bcl (cursor);
}
...

{
	...
	{
		col_append_sep_str (pinfo->cinfo, COL_INFO, ", ", val_to_str (homeplug_metype, homeplug_metype_vals, "Unknown 0x%x"));
	}
...
static void dissect_homeplug (tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree) 
...
/* Check we have enough data in tvb to read MEHDR */
			...
			if (check_tvb_length (cursor, HOMEPLUG_MEHDR_LEN) == TVB_LEN_SHORTEST) break;
			dissect_homeplug_mehdr (cursor);

/* Check we have enough data in tvb to read MELEN */

			if (check_tvb_length (cursor, HOMEPLUG_MELEN_LEN) == TVB_LEN_SHORTEST) break;
			dissect_homeplug_melen (cursor);
			dissect_homeplug_mme (cursor, pinfo);
		}
	}
	if (cursor) ptvcursor_free (cursor);
}
...
/* MAC Entry Header */
		...
		},
		{
			...
			{
				"MAC Entry Type",
				"homeplug.mehdr.metype",
				FT_UINT8,
				BASE_HEX,
				VALS (homeplug_metype_vals),
...
/* TODO must append vt[79-0] */
		...
		},
		{
			&hf_homeplug_cer_mod,
			{
				...
				FT_UINT8,
				BASE_DEC,
				VALS (homeplug_cer_mod_vals),
				HOMEPLUG_CER_MOD_MASK,
				NULL,
				HFILL
			}
		},
...
/* LOAD */
		...
		},
		{
			...
			{
				"Block Size",
				"homeplug.loader.nvmp.blocksize",
				FT_UINT32,
				BASE_DEC,
				NULL,
				0x0,
		...
		},
		{
			...
			{
				"Memory Size",
				"homeplug.loader.nvmp.memorysize",
				FT_UINT32,
				BASE_DEC,
				NULL,
				0x0,
...
/* Host Request */
		...
		},
		{
			...
			{
				"Get/Clear Local Bridge Proxy List",
				"homeplug.hreq.gclbpl",
				FT_UINT8,
				BASE_DEC,
				NULL,
				0x0,
		...
		},
		{
			...
			{
				"Get/Set Logical Network Mapping",
				"homeplug.hreq.gslnm",
				FT_UINT8,
				BASE_DEC,
				NULL,
...
/* Network Statistics */
		...
		},
		{
			...
			{
				"Network Statistics is Extended",
				"homeplug.ns.extended",
				FT_BOOLEAN,
				BASE_NONE,
				NULL,
				0x0,
...
/* ... Extended */
		...
		},
		{
			...
			{
				"Buffer in use",
				"homeplug.ns.buf_in_use",
				FT_BOOLEAN,
				8,
				TFS (&homeplug_ns_buf_in_use_vals),
				HOMEPLUG_NS_BUF_IN_USE,
		...
		},
		{
			...
			{
				"MSDU Length",
				"homeplug.ns.msdu_len",
				FT_UINT8,
				BASE_DEC,
				NULL,
				HOMEPLUG_NS_MSDU_LEN,
				NULL,
				HFILL
			}
		},
...
/* Bridging Characteristics Network */
		...
		},
		{
			...
			{
				"Network",
				"homeplug.bcn.network",
				FT_BOOLEAN,
				8,
				TFS (&homeplug_bc_network_vals),
				HOMEPLUG_BC_NETWORK,
...
/* Bridging Characteristics Local */
		...
		},
		{
			...
			{
				"Number of host proxied DAs",
				"homeplug.bcl.hprox_das",
				FT_UINT8,
				BASE_DEC,
				NULL,
				0x0,
...
/* Set Transmit Characteristics */
		...
		},
		{
			...
			{
				"Contention Free Transmit Override Priority",
				"homeplug.stc.cftop",
				FT_BOOLEAN,
				8,
				TFS (&homeplug_stc_cftop_vals),
				HOMEPLUG_STC_CFTOP,
